"""Made by Christopher John Bayron"""
"""
    Little bit Edit: Summer498
...
def _setup_chroma_vamp():
    ...
    for path in vamp_paths:
        try:
            ...
            print(f'autochord: Using NNLS-Chroma VAMP plugin in {path}', file=sys.stderr)
            return
...
def _download_model():
    ...
    model_files.sort()
    os.remove(model_zip)
    print(f'autochord: Chord model downloaded in {model_files[0]}', file=sys.stderr)
...
def _load_model():
    ...
    try:
        ...
        print(f'autochord: Loaded model from {_CHORD_MODEL_DIR}', file=sys.stderr)
    except Exception as e:
        raise Exception(f'autochord: Error in loading model: {e}')

def _init_module():
    print('autochord: Initializing...', file=sys.stderr)
    _setup_chroma_vamp()
    _load_model()
