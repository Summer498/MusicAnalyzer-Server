...
const bandpass = (a, x, b) => (b < x) ? b : (x < a) ? a : x;
...
const draw = () => {
    ...
    const chord_svg_elements = romans.map(time_and_roman => time_and_roman.progression.map((roman, i) => {
        ...
        const ret = roman.chord.notes.map((note) => {
    ...
    const piano_roll = SVG.svg({ name: "piano-roll", x: 0, y: 0, width: `${piano_roll_width}px`, height: `${piano_roll_height + slide_bar_param.height + chord_text_size}px`, }, undefined, [
        SVG.g({ name: "slide-bar", x: 0, y: 0, width: `${slide_bar_param.width}px`, height: `${slide_bar_param.height + chord_text_size}px` }, undefined, [slide_bar, slide_button]),
        Math.getRange(0, octave_cnt).map(i => SVG.g({ name: "octave", x: 0, y: `${slide_bar_param.height + octave_height * i}px`, width: `${piano_roll_width}px`, height: `${octave_height}px`, }, undefined, [
            white_position.map(j => SVG.rect({ x: 0, y: `${slide_bar_param.height + octave_height * i + black_key.height * j}px`, width: `${piano_roll_width}px`, height: `${black_key.height}px`, fill: white_back_fill, stroke: white_back_stroke, })),
            black_position.map(j => SVG.rect({ x: 0, y: `${slide_bar_param.height + octave_height * i + black_key.height * j}px`, width: `${piano_roll_width}px`, height: `${black_key.height}px`, fill: black_back_fill, stroke: black_back_stroke, })),
            chord_svg_elements.map(e => e.rect),
            chord_svg_elements.map(e => e.text),
            Math.getRange(0, 7).map(j => SVG.rect({ x: 0, y: `${slide_bar_param.height + octave_height * i + white_key.height * j}px`, width: `${white_key.width}px`, height: `${white_key.height}px`, fill: white_key.fill, stroke: white_key.stroke, })),
