...
Expected id is: ${regexp}`);
  ...
  var ChordPartSeries = class {
    constructor(id, controllers, children) {
      this.children = children;
      const svg = document.createElementNS("http://www.w3.org/2000/svg", "g");
      svg.id = id;
      children.forEach((e) => svg.appendChild(e.svg));
  ...
  var ChordKeySeries = class extends ChordPartSeries {
    remaining;
    constructor(romans, controllers) {
      super("key-names", controllers, romans.map((e) => new ChordKey(e)));
    }
  };
  var ChordNoteModel = class {
    constructor(e, note4, oct) {
      this.oct = oct;
      this.time = e.time;
      this.chord = e.chord;
      this.scale = e.scale;
      ...
      this.note = note4.chroma;
      this.note_name = note4.name;
      this.interval = distance4(this.tonic, note4);
    }
    time;
    chord;
    scale;
  ...
  var ChordNotesInOctave = class {
    ...
    constructor(roman, chord, oct) {
      ...
      const children = chord.notes.map((note4) => {
        const model = new ChordNoteModel(roman, get6(note4), oct);
  ...
  var ChordNotesSeries = class extends ChordPartSeries {
    constructor(romans, controllers) {
  ...
  var ChordNameSeries = class extends ChordPartSeries {
    constructor(romans, controllers) {
      const children = romans.map((e) => {
        ...
        svg.style.fontFamily = "Times New Roman";
        svg.style.fontSize = `${chord_text_em}em`;
        svg.style.fill = getColor3(model.tonic)(1, 0.75);
  ...
  var ChordRomanSeries = class extends ChordPartSeries {
    constructor(romans, controllers) {
      const children = romans.map((e) => {
        ...
        svg.style.fontFamily = "Times New Roman";
        svg.style.fontSize = `${chord_text_em}em`;
        svg.style.fill = getColor4(model.tonic)(1, 0.75);
  ...
  var DMelodyView = class {
    constructor(svg) {
      this.svg = svg;
    }
    set onclick(value) {
      this.svg.onclick = value;
    }
    updateX(x) {
      this.svg.setAttribute("x", String(x));
    }
  ...
  var DMelodySeries = class {
    ...
    onDMelodyVisibilityChanged(visible) {
  ...
  function buildDMelody(d_melody) {
  ...
  function buildIRPlot(h_melodies) {
  ...
  function buildIRSymbol(h_melodies) {
    const layers = h_melodies.map((e, l) => {
  ...
  function buildMelody(h_melodies) {
    const layers = h_melodies.map((e, l) => {
      const parts = e.map((e2) => {
  ...
  function buildReduction(h_melodies) {
  ...
  var GravityModel = class {
    constructor(layer, e, next, gravity) {
      this.next = next;
  ...
  var getInitPos = () => [0, 0, -triangle_width, +triangle_height, +triangle_width, +triangle_height];
  var GravityViewTriangle = class {
    constructor(svg) {
      this.svg = svg;
    }
    update(line_pos) {
  ...
  function buildGravity(mode, h_melodies) {
    const getLayers = (melodies, l) => {
      ...
      const gravity = next.map((n, i) => {
        ...
        const svg3 = getGravitySVG(triangle, line);
        const view = new GravityView(svg3, triangle, line);
        return {
          model,
          view,
          line_pos
        };
      }).filter((e) => e !== void 0).map((e) => new Gravity(e.model, e.view, e.line_pos));
      const svg2 = getSVGG6(`layer-${l}`, gravity);
      return new GravityLayer(l, svg2, gravity);
    };
  ...
  var setControllers = (d_melodies, melody, ir_symbol, ir_plot, chord_gravity, scale_gravity, reduction) => (controllers) => {
    ...
    const melodyBeepCheckBoxListeners = melody.children.flatMap((e) => e.children).flatMap((e) => e.onMelodyBeepCheckChanged.bind(e));
    controllers.melody_beep.checkbox.addListeners(...melodyBeepCheckBoxListeners);
    const melodyBeepVolumeListeners = melody.children.flatMap((e) => e.children).flatMap((e) => e.onMelodyVolumeBarChanged.bind(e));
 ...
 Given symbol:${symbol}`);
  ...
  var getArgsOfMelodyAnalysis = (args) => {
    if (args.length === 1) {
      const [e] = args;
      return [
        e.scale_gravity && new Gravity2(e.scale_gravity),
        e.chord_gravity && new Gravity2(e.chord_gravity),
