...
const piano_roll = SVG.svg(
  ...
  Math.getRange(0, octave_cnt).map((i) =>
    SVG.g(
      { name: "octave", x: 0, y: `${octave_height * i}px`, width: `${piano_roll_width}px`, height: `${octave_height}px`, },
      undefined,
      [
        white_position.map((j) =>
          SVG.rect({ x: 0, y: `${black_key.height * j + octave_height * i}px`, width: `${piano_roll_width}px`, height: `${black_key.height}px`, fill: white_back_fill, stroke: white_back_stroke, })
        ),
        black_position.map((j) =>
          SVG.rect({ x: 0, y: `${black_key.height * j + octave_height * i}px`, width: `${piano_roll_width}px`, height: `${black_key.height}px`, fill: black_back_fill, stroke: black_back_stroke, })
        ),
        Math.getRange(0, 7).map((j) =>
          SVG.rect({ x: 0, y: `${white_key.height * j + octave_height * i}px`, width: `${white_key.width}px`, height: `${white_key.height}px`, fill: white_key.fill, stroke: white_key.stroke, })
        ),
        black_position.map((j) =>
          SVG.rect({ x: 0, y: `${black_key.height * j + octave_height * i}px`, width: `${black_key.width}px`, height: `${black_key.height}px`, fill: black_key.fill, stroke: black_key.stroke, })
        ),
      ]
    )
  )
