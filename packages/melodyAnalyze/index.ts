...
const getTimeAndChord = (chord_strs: TimeAndString[]) => {
  ...
  const non_null_chord = (() => {
    ...
    time_and_chord.forEach((e) =>
      e.chord.empty ? 0 : res.push({ time: e.time, chord: e.chord }),
...
const analyzeMelody = (
  ...
  return melodies.map((melody, i) => {
    ...
    const roman = romans.find(
      (roman) =>
        roman.time[0] <= melody.time[1] && melody.time[0] < roman.time[1],
    ); // TODO: 治す. 現状はとりあえずコードとメロディを大きめに重ならせてみているだけ
    if (roman) {
...
const main = (argv: string[]) => {
  ...
  const melody_csv = parse_csv(melody_txt).map((e) =>
    isNaN(e) ? null : Math.round(freqToMidi(e)),
  ); // compress が NaN を全て別のオブジェクト扱いするので null に置換する
  const comp_melody = compress(melody_csv);
  const non_null_melody = (() => {
    const res: { time: number[]; note: number }[] = [];
    comp_melody.forEach((e) =>
      e.value === null? 0: res.push({ time: e.time.map((i) => i / melody_sr), note: e.value }),
