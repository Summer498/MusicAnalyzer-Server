import { ChordProgression } from "../key-estimation";
import { remove_item } from "./remove-item";
import { select_suitable_progression } from "./select-suitable-progression";
import { splitArray } from "./split-array";
import { TimeAndString } from "./time-and-chord";
import { TimeAndRomanAnalysis } from "./time-and-roman-analysis";

// Expected Input: "Am7 FM7 G7 CM7"
export const calcChordProgression = (chords: TimeAndString[]): TimeAndRomanAnalysis[] => {
  const tmp0 = splitArray(chords, chord => chord[2] === "N"); // ノンコードシンボルを除く     ["C", "F", "N", "N", "G","C"]       => [["C"],["F"], [], ["G"],["C"]]
  const time_and_chord_groups = remove_item(tmp0, item => item.length === 0); // 空配列を除く                 [["C"],["F"], [], ["G"],["C"]]      => [["C","F"], ["G","C"]]

  return time_and_chord_groups.flatMap(time_and_chords => {
    const time = time_and_chords.map(chord => ({
      begin: Math.floor(chord[0] * 1000) / 1000,
      end: Math.floor(chord[1] * 1000) / 1000
    }));
    const progression = select_suitable_progression(
      new ChordProgression(time_and_chords.map(chord => chord[2])).getMinimumPath(),
    );
    return time_and_chords.map((_, i) => {
      return {
        begin: time[i].begin,
        end: time[i].end,
        roman: progression[i].roman,
        chord: progression[i].chord.name,
        scale: progression[i].scale.name,
      };
    });
  });
};
