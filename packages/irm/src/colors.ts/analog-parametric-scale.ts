...
const get_grb_on_parametric_scale = (archetype: Archetype) => {
  const s = archetype.intervallic_motion.direction.name === "mL" ? -1 : 0;
  const v = archetype.intervallic_motion.direction.name === "mR" ? -1 : 0;
  const scale = archetype.intervallic_motion.magnitude.name === "AA" ? 0.25 : 0.5;
  const B = 120;
  switch (archetype.registral_motion.direction.name) {
