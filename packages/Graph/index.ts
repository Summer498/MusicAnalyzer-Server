...
const argsMinMax = <T>(f: (i: T) => number, space: Array<T>, compare: Compare) => {
    let val = compare(0, 1) ? Infinity : -Infinity;
    let args: T[] = [];
    for (const e of space) {
        const v = f(e);
        if (compare(v, val)) { val = v; args = [e]; }
        else if (val === v) { args.push(e); }
    }
    return { val, args };
}
...
const _dynamicLogViterbi = (
    ...
    A: (prev_time: number, time: number, prev_state: number, state: number) => number,
    ...
    let z = newArray(N).map(_=>newArray<number>(T))  // state_trace_paths
    for (let i = 0; i < N; i++) {
        z[i][T - 1] = zn_T[i]
    }
